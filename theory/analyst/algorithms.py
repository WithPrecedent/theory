...
class CompareCleaves(TechniqueOutline):
    ...
    def add(self, cleave_group, columns):
        """For the cleavers in theory, this step alows users to manually
...
class Cookbook(Book):
    ...
    Args:
        name (Optional[str]): designates the name of the class used for internal
            referencing throughout theory. If the class needs settings from
            the shared 'Idea' instance, 'name' should match the appropriate
            section name in 'Idea'. When subclassing, it is a good idea to use
            the same 'name' attribute as the base class for effective
            coordination between theory classes. 'name' is used instead of
...
class Tool(Technique):
    ...
    Args:
        ...
        module (Optional[str]): name of module where object to use is located
            (can either be a theory or non-theory module). Defaults to
            'theory.core'.
    ...
    def _get_model_type(self, data: 'Dataset') -> str:
        ...
        Raises:
            TypeError: if 'label' attribute is neither None, 'boolean',
                'category', 'integer' or 'float' data type (using theory
                proxy datatypes).
